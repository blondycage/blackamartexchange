...
class bitchart extends Component {
  constructor() {
    super();
    this.state = {
      data: {}
      ...
      .then(res => {
        ...
        for (let date in res.data.bpi) {
          sortedData.push({
            ...
            p: res.data.bpi[date].toLocaleString('us-EN', {
              style: 'currency',
              currency: 'USD'
            }),
    ...
    return (
      <div style={{ padding: '0 150px', maxHeight: '500px'  }} className="App">
        ...
        <Chart
          ...
          legendText="© to Har8"
        />
        <div style={{ color: '#70747f' }}>
          {' '}
          <span style={{ color: '#222' }}>&copy;</span> to Har8
        </div>
      </div>
    );
  }
